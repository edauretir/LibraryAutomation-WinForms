...
{
    public partial class KitapRezervasyon : Form
    {
        public KitapRezervasyon()
        {
            InitializeComponent();
        }
        ...
        {
            baglanti.Open();
            SqlDataAdapter adtr = new SqlDataAdapter("select *from sepet2", baglanti);
            adtr.Fill(daset, "sepet2");
            dataGridView1.DataSource = daset.Tables["sepet2"];
            baglanti.Close();
        }
        ...
        {
            baglanti.Open();
            SqlCommand komut = new SqlCommand("insert into sepet2(barkodno,kitapadi,yazari,yayinevi,sayfasayisi,kitapsayisi,teslimtarihi,iadetarihi) values(@barkodno,@kitapadi,@yazari,@yayinevi,@sayfasayisi,@kitapsayisi,@teslimtarihi,@iadetarihi)", baglanti);
        ...
        {
            baglanti.Open();
            SqlCommand komut = new SqlCommand("select sum(kitapsayisi) from sepet2", baglanti);
            lblKitapSayi.Text = komut.ExecuteScalar().ToString();
            baglanti.Close();
        }

        private void KitapRezervasyon_Load(object sender, EventArgs e)
        {
            sepetlistele();
            kitapsayisi();
        }
        ...
        private void txtTcAra_TextChanged(object sender, EventArgs e)
        {
            ...
            baglanti.Open();
            SqlCommand komut2 = new SqlCommand("select sum(kitapsayisi) from rezerveedilenkitaplar where tc='" + txtTcAra.Text + "'", baglanti);
        ...
        private void btnSil_Click(object sender, EventArgs e)
        {
            ...
            {
                baglanti.Open();
                SqlCommand komut = new SqlCommand("delete from sepet2 where barkodno='" + dataGridView1.CurrentRow.Cells["barkodno"].Value.ToString() + "'", baglanti);
                komut.ExecuteNonQuery();
                baglanti.Close();
        ...
        private void btnRezerveEt_Click(object sender, EventArgs e)
        {
            if (lblKitapSayi.Text != "")
            {
                ...
                {
                    if (txtTcAra.Text != "" && txtAd.Text != "" && txtSoyad.Text != "" && dateTimePicker3.Text != "" && txtTelefon.Text != "")
                    {
                        for (int i = 0; i < dataGridView1.Rows.Count - 1; i++)
                        {
                            baglanti.Open();
                            SqlCommand komut = new SqlCommand("insert into rezerveedilenkitaplar(tc,ad,soyad,dogtar,telefon,barkodno,kitapadi,yazari,yayinevi,sayfasayisi,kitapsayisi,teslimtarihi,iadetarihi) values(@tc,@ad,@soyad,@dogtar,@telefon,@barkodno,@kitapadi,@yazari,@yayinevi,@sayfasayisi,@kitapsayisi,@teslimtarihi,@iadetarihi)", baglanti);
                            ...
                            komut.ExecuteNonQuery();
                            SqlCommand komut2 = new SqlCommand("update uye set okukitapsayisi=okukitapsayisi+'" + int.Parse(dataGridView1.Rows[i].Cells["kitapsayisi"].Value.ToString()) + "'where tc='" + txtTcAra + "'", baglanti);
                        ...
                        baglanti.Open();
                        SqlCommand komut4 = new SqlCommand("delete from sepet2", baglanti);
                        komut4.ExecuteNonQuery();
                        baglanti.Close();
                        MessageBox.Show("Kitaplar rezerve edildi");
                        daset.Tables["sepet2"].Clear();
                        sepetlistele();
                        txtTcAra.Text = "";
                        lblKitapSayi.Text = "";
